...
import time
import sys
from qtui import mainmenu, miss_ingredients, pump_set_ui, drink_list, pouring, manual_pour, barstock
from xml.etree import ElementTree
import pump_setup
import threading

AppLocation = '/Users/Leandro Teles/Dropbox/Programming/Drink_Machine/'
...
class MainWindow(QtWidgets.QMainWindow, mainmenu.Ui_MainMenu):
    def __init__(self):
        ...
        self.background.setBrush(QtGui.QPalette.Background,
                                 QtGui.QBrush(QtGui.QPixmap(AppLocation + 'images/MainScreen.jpg')))
        ...
        self.btn_bar.clicked.connect(lambda: BarStock.show())
...
class DrinksList(QtWidgets.QWidget, drink_list.Ui_DrinkList):
    def __init__(self):
        ...
        self.drink_menu.setCurrentRow(0)
        ...
        self.btn_up.clicked.connect(lambda: self.step_selection('up', 1))
        self.btn_down.clicked.connect(lambda: self.step_selection('down', 1))
        self.btn_pgup.clicked.connect(lambda: self.step_selection('up', 20))
        self.btn_pgdown.clicked.connect(lambda: self.step_selection('down', 20))
    ...
    # Use the large touchscreen buttons to step up or down the list of drinks.
    def step_selection(self, direction, steps):
        cur_index = self.drink_menu.currentRow()
        max_index = len(self.drink_menu) - 1
        if direction == 'up':
            new_index = cur_index - steps
            if new_index < 0:
                new_index = 0
            self.drink_menu.setCurrentRow(new_index)
        elif direction == 'down':
            new_index = cur_index + steps
...
class PrepWindow(QtWidgets.QWidget, pouring.Ui_PourWindow):
    ...
    # Prepare to serve the selected drink.
...
class ProgressWindow(QtWidgets.QWidget, miss_ingredients.Ui_MissingIngredients):
    def __init__(self):
    ...
    def populate(self):
        self.find_missing()
        if self.missing == []:
            Progress.label.setText('There are no missing ingredients.\n'
                                   'Your drink will be ready in a moment.')
        else:
            Progress.label.setText('The following ingredients are not in the machine.\n'
                                   'Please add them to your drink according to the instructions.')

        ing_list = ''
        for i in self.missing:
            ing_list += i[0] + ' - ' + str(float(i[1]) * Prep.multiplier) + '\n'
        self.dsp_miss_ingredients.setText(ing_list)
        self.dsp_instructions.setText(Prep.instruction)

    def find_missing(self):
        self.missing = []
        for i in Prep.ingredients:
            found = False
            for p in Pumps.all_pumps:
                if i[0] == p.booze:
                    found = True
...
class PumpSetup(QtWidgets.QWidget, pump_set_ui.Ui_PumpSettings):
    def __init__(self):
        ...
        self.btn_calibrate.clicked.connect(lambda: threading.Thread(target=self.calibrate).start())
...
class MyBar(QtWidgets.QWidget, barstock.Ui_BarStock):
    def __init__(self):
